...
import StepContainer from '../../accommodationForm/components/StepContainer'
...
const AccommodationForm = ({ form }: { form: PropsMultiStepForm }) => {
    ...
    const methods = useForm<AccommodationFormData>({ resolver: zodResolver(form.schema), mode: "onChange" })
            ...
            <FormProvider {...methods}>
                <form className='w-full flex  flex-col gap-4' onSubmit={onSubmit}>
                    ...
                    <div className='flex  justify-between items-center '>
                        <button type='button' onClick={handleBack} className='px-7 bg-primary py-3 rounded-xl  text-secondary font-semibold shadow-md text-roboto hover:opacity-80   transition-opacity duration-300'>
                            {!form.isFirstStep ? 'Back' : "Cancel"}
                        </button>

                        <button type='submit' disabled={!methods.formState.isValid || loading} className='px-7 bg-primary py-3 rounded-xl  disabled:opacity-50 text-secondary font-semibold shadow-md text-roboto hover:opacity-80   transition-opacity duration-300'>
